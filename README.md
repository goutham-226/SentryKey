...
**Schema changes go through migrations.** Every table is defined by a reversible,
reviewed migration. No table is altered by hand, in any environment.
...
**Configuration and resources arrive by dependency injection.** Settings, the database
session, and the authenticated key are all resolved through `Depends`, which keeps
handlers free of setup code and makes every one of them substitutable under test.
